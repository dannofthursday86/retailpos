...
# Dummy routes - define each separately to avoid closure issues
@app.route('/public/selling/cart')
def selling_cart():
    if not session.get('logged_in'): return redirect(url_for('login'), code=302)
    return render_template_string(BASE.replace('{% block content %}{% endblock %}', '<div class="table-container"><div class="table-header"><h3><i class="fas fa-shopping-cart"></i> POS</h3></div><p style="padding:20px;">Coming soon...</p></div>').replace('{% block page_title %}', '<i class="fas fa-shopping-cart"></i> POS - ').replace('{% endblock %}', ''), stats=stats, current_date=datetime.datetime.now().strftime('%d-%m-%Y'))

@app.route('/public/quote')
def quote():
    if not session.get('logged_in'): return redirect(url_for('login'), code=302)
    return render_template_string(BASE.replace('{% block content %}{% endblock %}', '<div class="table-container"><div class="table-header"><h3><i class="fas fa-file-alt"></i> Quotation</h3></div><p style="padding:20px;">Coming soon...</p></div>').replace('{% block page_title %}', '<i class="fas fa-file-alt"></i> Quotation - ').replace('{% endblock %}', ''), stats=stats, current_date=datetime.datetime.now().strftime('%d-%m-%Y'))

@app.route('/public/selling/indexMinyak')
def minyak():
    if not session.get('logged_in'): return redirect(url_for('login'), code=302)
    return render_template_string(BASE.replace('{% block content %}{% endblock %}', '<div class="table-container"><div class="table-header"><h3><i class="fas fa-oil-can"></i> Minyake</h3></div><p style="padding:20px;">Coming soon...</p></div>').replace('{% block page_title %}', '<i class="fas fa-oil-can"></i> Minyake - ').replace('{% endblock %}', ''), stats=stats, current_date=datetime.datetime.now().strftime('%d-%m-%Y'))

@app.route('/public/redeempoint')
def redeempoint():
    if not session.get('logged_in'): return redirect(url_for('login'), code=302)
    return render_template_string(BASE.replace('{% block content %}{% endblock %}', '<div class="table-container"><div class="table-header"><h3><i class="fas fa-gift"></i> Redeem Point</h3></div><p style="padding:20px;">Coming soon...</p></div>').replace('{% block page_title %}', '<i class="fas fa-gift"></i> Redeem Point - ').replace('{% endblock %}', ''), stats=stats, current_date=datetime.datetime.now().strftime('%d-%m-%Y'))

@app.route('/public/cheque')
def cheque():
    if not session.get('logged_in'): return redirect(url_for('login'), code=302)
    return render_template_string(BASE.replace('{% block content %}{% endblock %}', '<div class="table-container"><div class="table-header"><h3><i class="fas fa-money-check"></i> Cheque</h3></div><p style="padding:20px;">Coming soon...</p></div>').replace('{% block page_title %}', '<i class="fas fa-money-check"></i> Cheque - ').replace('{% endblock %}', ''), stats=stats, current_date=datetime.datetime.now().strftime('%d-%m-%Y'))

@app.route('/public/returns')
def returns():
    if not session.get('logged_in'): return redirect(url_for('login'), code=302)
    return render_template_string(BASE.replace('{% block content %}{% endblock %}', '<div class="table-container"><div class="table-header"><h3><i class="fas fa-undo"></i> Return</h3></div><p style="padding:20px;">Coming soon...</p></div>').replace('{% block page_title %}', '<i class="fas fa-undo"></i> Return - ').replace('{% endblock %}', ''), stats=stats, current_date=datetime.datetime.now().strftime('%d-%m-%Y'))

@app.route('/public/inventory/experied')
def experied():
    if not session.get('logged_in'): return redirect(url_for('login'), code=302)
    return render_template_string(BASE.replace('{% block content %}{% endblock %}', '<div class="table-container"><div class="table-header"><h3><i class="fas fa-exclamation-triangle"></i> Expired Stock</h3></div><p style="padding:20px;">Coming soon...</p></div>').replace('{% block page_title %}', '<i class="fas fa-exclamation-triangle"></i> Expired Stock - ').replace('{% endblock %}', ''), stats=stats, current_date=datetime.datetime.now().strftime('%d-%m-%Y'))

@app.route('/public/admin/versiapp')
def versiapp():
    if not session.get('logged_in'): return redirect(url_for('login'), code=302)
    return render_template_string(BASE.replace('{% block content %}{% endblock %}', '<div class="table-container"><div class="table-header"><h3><i class="fas fa-info-circle"></i> Versi Aplikasi</h3></div><p style="padding:20px;">Version 3.1.0</p></div>').replace('{% block page_title %}', '<i class="fas fa-info-circle"></i> Versi Aplikasi - ').replace('{% endblock %}', ''), stats=stats, current_date=datetime.datetime.now().strftime('%d-%m-%Y'))

@app.route('/public/admin/qr')
def qr_inventory():
    if not session.get('logged_in'): return redirect(url_for('login'), code=302)
    return render_template_string(BASE.replace('{% block content %}{% endblock %}', '<div class="table-container"><div class="table-header"><h3><i class="fas fa-qrcode"></i> QR Inventory</h3></div><p style="padding:20px;">Coming soon...</p></div>').replace('{% block page_title %}', '<i class="fas fa-qrcode"></i> QR Inventory - ').replace('{% endblock %}', ''), stats=stats, current_date=datetime.datetime.now().strftime('%d-%m-%Y'))
